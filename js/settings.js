// ══ settings.js — Full Synced Version ══
...
export function renderUsersList() {
  ...
  if (!el) return;
  const ROLE_LBL = { owner: '👑 בעלים', admin: '🔧 אדמין', tech: '🔧 טכנאי' };
  el.innerHTML = USERS.map(u => `
    <div style="display:flex;align-items:center;gap:10px;padding:10px 12px;background:var(--sur2);border-radius:8px;margin-bottom:8px">
      ...
      <div style="flex:1">
        ...
        <div style="font-size:11px;color:var(--tx3)">${ROLE_LBL[u.role] || u.role}</div>
      </div>
      <button class="btn bs btn-sm" onclick="window._openEditUser('${u.name}')">✏️</button>
    </div>`).join('');
}
...
export function openEditUser(name) {
  ...
  renderPermsGrid(getPerms(u.name)); // שונה לשימוש בשם
  openM('M-user');
}
...
export function renderPermsGrid(perms) {
  ...
  grid.innerHTML = PERM_MODULES.map(m => `
    <div style="background:var(--sur2);border-radius:8px;padding:8px 10px">
      ...
      <select class="finp perm-sel" data-key="${m.key}">
